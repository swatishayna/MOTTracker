...
def draw_boxes(frame, bbox_xyxy, draw_trails=None, identities=None, categories=None, offset=(0,0)):
...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            ims = im0.copy()
            if len(det):
                frame_info["Frame "+str(frame)+ ":"] = len(det)
                ...
                frame_info[i] = {str(len(det)): len(xywh_bboxs)}
            ...
            if save_img:
                if vid_path[i] != save_path:  # new video
                   ...
                   # if str(source).split(".")[-1] == ".mp4":
                    save_path = str(Path(save_path).with_suffix('.mp4'))  # force *.mp4 suffix on results videos
                    vid_writer[i] = cv2.VideoWriter(save_path, cv2.VideoWriter_fourcc(*'mp4v'), fps, (w, h))
                vid_writer[i].write(ims)

            if len(det) > 0 and len(outputs) == 0:

                desired_width = 800
                desired_height = 600
                resized_image = cv2.resize(ims, (desired_width, desired_height))

                count = 0
                for *xyxy, conf, cls in reversed(det):
                    classNameInt = int(cls)
                    color = colorLabels(classNameInt)
                    x1, y1, x2, y2 = xyxy
                    x1, y1, x2, y2 = int(x1), int(y1), int(x2), int(y2)
                    cv2.rectangle(ims, (x1, y1), (x2, y2), color, 2)

                    name = className[classNameInt]
                    id_name = str(count) + ":" + name

                    text_size = cv2.getTextSize(id_name, 0, fontScale=0.25, thickness=2)[0]
                    c2 = x1 + text_size[0], y1 - text_size[1] - 3
                    center = int((x1 + x2) / 2), int((y1 + y2) / 2)
                    cv2.rectangle(ims, (x1, y1), c2, color, -1)
                    cv2.putText(ims, id_name, (x1, y1 - 2), 0, 0.25, [255, 255, 255], thickness=1, lineType=cv2.LINE_AA)
                    cv2.circle(ims, center, 2, (0, 255, 0), cv2.FILLED)
                    count = count + 1

                output_path = "bb" + source
                # Define the text to be added and its position
                text = "Total Object: " + str(int(len(det)))
                ...
                bottom_left_corner_of_text = (10, ims.shape[0] - 10)
                top_left_corner_of_text_box = (bottom_left_corner_of_text[0], bottom_left_corner_of_text[1] - text_height - baseline)
                bottom_right_corner_of_text_box = (bottom_left_corner_of_text[0] + text_width, bottom_left_corner_of_text[1])

                # Draw a filled rectangle as the background for the text
                rectangle_bgr = (0, 255, 0)  # Green color
                cv2.rectangle(ims, top_left_corner_of_text_box, bottom_right_corner_of_text_box,
                              rectangle_bgr, cv2.FILLED)
                cv2.putText(ims, text, bottom_left_corner_of_text, fontFace=0, fontScale=0.25, color=[0,0,0], thickness=1, lineType=cv2.LINE_AA)
                cv2.imwrite(output_path, ims)
